...
import argparse
from data_load_ddi import load_interaction_data
...
parser.add_argument('--diff_w', type=bool, default=True, help='diff_w')
...
if diff_w:
    ...
    import layers_new_same_w as mylayers
...
restore_path = 'model_save/test.para_test9_matmul_coLSTM-conv384-dense128-t_size200-hid64-drop2None-learn0.0001-beta0.001-data-ddi-batch512-concat-rFalse-4.16-18'
...
def evaluate(dataset, steps, lose_num):
    total_tags = 0.0
    correct_tags = 0.0
    predic,y_test, auc_pred_set, loss_sum = [],[], [], 0
    ...
    for i in range(steps):
        ...
        total_tags += num_y
        correct_tag, predic_, y_ = sess.run(
            (correct, predictions, label_gold), feed_dict=dataset[i])
        predic+=list(predic_[:num_y])
        y_test += list(y_[:num_y])
    ...
    accuracy=metrics.accuracy_score(y_test, predic)
    macro_recall = metrics.recall_score(y_test, predic,average='macro')
    micro_recall = metrics.recall_score(y_test, predic,average='micro')
    macro_precision = metrics.precision_score(y_test, predic,average='macro')
    micro_precision = metrics.precision_score(y_test, predic,average='micro')
    macro_f1=metrics.f1_score(y_test, predic,average='macro')
    micro_f1=metrics.f1_score(y_test, predic,average='micro')

    def one_hot(y, C):
        return np.eye(C)[y.reshape(-1)].T

    try:
        y_test = one_hot(np.array(y_test), n_class)
...
with tf.Graph().as_default():
    ...
    #                   dtype=tf.float32)
    w_1 = tf.get_variable('w_1', shape=[hidden_size, n_class], dtype=tf.float32,
                          initializer=tf.contrib.layers.xavier_initializer())

    b_1 = tf.get_variable("b_1", [n_class], dtype=tf.float32)
    ...
    ########################################################################################################################
    # data loader
    _, train_A, train_B = load_interaction_data(filename=dataset + '_train_cleaned')
    _, valid_A, valid_B = load_interaction_data(filename=dataset + '_valid_cleaned')
    _, test_dataset_A, test_dataset_B = load_interaction_data(filename=dataset + '_test_cleaned')
    ...
    if mode=='training':
        ...
        def gen_dict(a_train,b_train,train_step,training_value=0):
            feed_dict=[]
            for i in range(train_step):
                ...
                feed_dict.append(merge_dicts([label_dict, atoms_dict, training_dict]))
            return feed_dict


        feed_dict_train=gen_dict(a_train,b_train,train_step,1)
        feed_dict_train_reverse=gen_dict(b_train,a_train,train_step,1)
        ...
        feed_dict_valid = gen_dict(a_valid, b_valid, valid_step)
        feed_dict_test = gen_dict(a_test, b_test, test_step)

        accuracy, evaluation = evaluate(feed_dict_test,test_step, lose_num_test)
        print("Accuracy for test set is", accuracy, evaluation)
        for epoch in range(max_epochs):
            print('epoch %d' % (epoch))
            for i in range(train_step):
                _, loss_value = sess.run([train_op, loss], feed_dict=feed_dict_train[i])
                total_loss += loss_value
                if reverse:
                    _, loss_value = sess.run([train_op, loss], feed_dict=feed_dict_train_reverse[i])
                    total_loss += loss_value
                if (i+1) == train_step:
                    ...
                    if (epoch%5) == 4 or losses <= 1000:
                        ...
                        accuracy_valid, evaluation_valid = evaluate(feed_dict_valid, valid_step, lose_num_valid)
                        print("Current accuracy is " ,accuracy_valid, evaluation_valid)
