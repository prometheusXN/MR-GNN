...
filename = 'test.{}-conv{}-dense{}-t_size{}-hid{}-drop2{}-learn{}-beta{}-data-{}-batch{}-{}-r{}-{}.{}-{}'.format \
    (model_name, conv_size, Dense_size, tensor_size, hidden_size, dropout2, learning_rate, beta, dataset, batch_size,
     merge_mode, reverse,
...
with tf.Graph().as_default():
    ...
    # GraphConv_and_gather Layer outputs the gather vector of the upstream graphs and the new atom feature after GCN.
    # lay1_norm = dc.nn.BatchNormalization(epsilon=1e-5, mode=1)
    lay1_pool = mylayers.GraphPool()
    lay1_gather = mylayers.Gather1(conv_size, n_features, batch_size, activation='relu', dropout=dropout0)
    ...
    outputB, gatherB, _ = lay1_conv([outputB] + graphB_topology.get_topology_placeholders() + [training] + [add_time])
    ...
    outputA, gatherA, _ = lay2_conv([outputA] + graphA_topology.get_topology_placeholders() + [training] + [add_time])
    ...
    outputB, gatherB, _ = lay2_conv([outputB] + graphB_topology.get_topology_placeholders() + [training] + [add_time])
    ...
    #---------------------------------------------layer3----------------------------------------#
    lay3_conv = mylayers.GraphConv_and_gather(conv_size, conv_size, batch_size, activation='relu', dropout=dropout2)
    # lay4_norm = dc.nn.BatchNormalization(epsilon=1e-5, mode=1)
    lay3_dense = tf.keras.layers.Dense(Dense_size, activation='relu')

    outputA, gatherA, poolingA = lay3_conv([outputA] + graphA_topology.get_topology_placeholders() + [training] + [add_time])
    gatherA = lay3_dense(gatherA)
    # gatherA = lay4_norm(gatherA)
    h_A_3, stateA = lstm(gatherA, states=stateA)

    outputB, gatherB, poolingB = lay3_conv([outputB] + graphB_topology.get_topology_placeholders() + [training] + [add_time])
    gatherB = lay3_dense(gatherB)
    # gatherB = lay4_norm(gatherB)
    h_B_3, stateB = lstm(gatherB, states=stateB)

    Inter = tf.concat([gatherA, gatherB], 1)
    h_3, State = lstm_1(Inter, states=State)

    #---------------------------------------------layer_final----------------------------------------#
    lay_final_gather = mylayers.Gather1(conv_size, conv_size, batch_size, activation='relu', dropout=dropout2)
    lay4_dense = tf.keras.layers.Dense(Dense_size, activation='relu')

    gatherA = lay_final_gather([outputA] + graphA_topology.get_topology_placeholders() + [training] + [add_time])
    gatherB = lay_final_gather([outputB] + graphB_topology.get_topology_placeholders() + [training] + [add_time])
    ...
    w_0 = tf.get_variable('w_0', shape=[fully_size, hidden_size], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
